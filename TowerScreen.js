...
const TowerScreen = ({navigation}) => {
    ...
    const intervalRef = useRef(10000); // Track current interval, start from 10 seconds
    const rowsAddedAtCurrentInterval = useRef(0) // Track num of rows at the current interval
    const [isPaused, setIsPaused] = useState(false); // Track if game is paused
    const animationRefs = useRef([]); // Store references to animations
    const [showHelpModel, setShowHelpModal] = useState(false);
    ...
    useEffect(() => {
        ...
        setDifficultyLevel(prev => Math.min(prev + 1, 3)); // Cap at max difficulty (3)
        }
    }, [score]);
    ...
    useEffect(() => {
        ...
        animationRefs.current = initialAnimations.map((anim) => anim); // Store animation references
    }, [settings.animationsEnabled]);

    // Initialise animations for the initial rows
    useEffect(() => {
        if (isPaused) return; // If paused, pause animation
    ...
    }, [fallAnimations, isPaused, settings.animationsEnabled]);

    // Add a new row with dynamic intervals
    useEffect(() => {
        if (gameOver || isPaused) return; // Stop adding rows if game is over or paused
        ...
        const startInterval = () => {
            intervalId = setInterval(() => {
                ...
                // Update total rows and total rows added at current interval
                console.log(`Adding new row at interval: ${intervalRef.current}ms`); // Log intervaL
                setRowsCount((prevCount) => prevCount + 1);
                rowsAddedAtCurrentInterval.current += 1;
    
                // After 5 rows at the current interval, decrease the interval by 1 second
                if (rowsAddedAtCurrentInterval.current >= 5) {
                    if (intervalRef.current > 3000) { // Ensure interval is minimum of 3 seconds
                        intervalRef.current -= 1000; // Decrease interval by 1 second
                    }
                    rowsAddedAtCurrentInterval.current = 0; // Reset the counter
    
                    // Clear existing interval and start a new one with the updated interval
        ...
        // Clean up interval on component unmount or when gameOver changes
    ...
    useEffect(() => {
        if (rowsCount > 4) {
            ...
            // If not paused, start the animation for the new row
            if (!isPaused) {
    ...
    return (
        <View style={dynamicStyles.container}>
            ...
            >
                <Text style={dynamicStyles.headerTitle}>TOWER</Text>
            ...
            <View style={dynamicStyles.main}>
                {Array.from({ length: rowsCount })
                    ...
                    .map((index) => (
                        ...
                        >
                            <WordBlocks onCorrectUnscramble={(word) => handleCorrectUnscramble(index, word)} difficultyLevel={Math.min(difficultyLevel + Math.floor(index/3), 3)} />
                        </Animated.View>
                    ))
                }
            </View>
            ...
            {/** Pause Modal */}
            <Modal transparent={true} visible={isPaused && !showHelpModel}>
            ...
            {/** Help Modal */}
            <Modal transparent={true} visible={showHelpModel}>
                <View style={dynamicStyles.pauseOverlay}>
                    <View style={dynamicStyles.translucentBackground} />
                    <View style={dynamicStyles.helpModalContent}>
                        ...
                        <Text style={dynamicStyles.helpModalText}>
                            ...
                            {"\n"}
                            4. The game ends when a row reaches the top.
                        </Text>
            ...
            <GameOverModal
                ...
                score={score}
                onPlayAgain={async () => {
                    const achievements = await updateStats("Tower", score, wordsFound);
                    setUnlockedAchievements(achievements);
                    setShowGameOverModaL(false);
                    if (achievements.length > 0) {
                        setShowAchievementModal(true);
                    } else {
                        navigation.replace("Tower");
                    }
                }}
                onHome={async () => {
                    const achievements = await updateStats("Tower", score, wordsFound);
